...
if __name__ == "__main__":
    ...
    retrain_logits = df_logits[retrain_index]
    retrain_test_logits = df_logits_test[retrain_index]
    ...
    for i, model_logits in enumerate(df_logits):
        ...
        kl_div = F.kl_div(retrain_probs.log(), model_probs, reduction='batchmean')

        # Compute Absolute Difference
        abs_diff = torch.abs(retrain_logits - model_logits)
    ...
    # now compare against test samples of poisoned class
    results = []
    for i, model_logits in enumerate(df_logits_test):
        ...
        # KL Divergence computation (logits to probabilities)
        retrain_probs = F.softmax(retrain_test_logits, dim=-1)
        model_probs = F.softmax(model_logits, dim=-1)
        ...
        # Compute Absolute Difference
        abs_diff = torch.abs(retrain_test_logits - model_logits)
        abs_diff_mean = abs_diff.mean().item()
