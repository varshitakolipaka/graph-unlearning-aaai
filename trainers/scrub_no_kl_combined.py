...
class ScrubTrainer2(Trainer):
    ...
    def unlearn_nc_lf(self):
        ...
        # Initialize a single scheduler (if needed)
        ...
        while self.curr_step < self.opt.unlearn_iters:
            ...
            # Compute the descent (remembering) loss
            ...
            # Combine the ascent and descent losses
            # PLS NOTE: alpha is being used in an unrelated way in this version of scrub. here it basically means ascent weight
            total_loss = self.opt.scrubAlpha * forget_loss + total_descent_loss
            total_loss.backward()
            self.optimizer.step()
            scheduler.step()
