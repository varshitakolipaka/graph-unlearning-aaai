    ...
    def unlearn_nc_lf(self):
        ...
        while self.curr_step < self.opt.unlearn_iters:
            ...
            if self.curr_step % 1 == 0:
                ...
                # Evaluate with validation set and get scores
                self.evaluate(use_val=True)
                val_forg, val_util = self.get_score(self.opt.attack_type, class1=class_dataset_dict[self.opt.dataset]["class1"], class2=class_dataset_dict[self.opt.dataset]["class2"])
                
                # Calculate the mean of forgetting ability and utility
                current_score = (val_forg + val_util) / 2

                # Update best model if necessary
                if current_score > self.best_score:
                    self.best_score = current_score
                    print("Best model so far!")
                    print(f"New best score: {self.best_score:.4f} (Forget: {val_forg:.4f}, Utility: {val_util:.4f})")
                    with open(self.opt.unlearning_model + '_best_model.pth', 'wb') as f:
                        torch.save(self.model.state_dict(), f)
