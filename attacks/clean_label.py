...
def clean_label(data, epsilon, seed, victim_class=68, trigger_size=15, test_poison_fraction=0.5):
    data = data.cpu()
    ...
    # Now handle test set poisoning (non-victim class only)
    test_mask = poisoned_data.test_mask
    victim_test_indices = [
        i for i in range(poisoned_data.num_nodes)
        if test_mask[i] and poisoned_data.y[i] != victim_class  # Poison only non-victim class nodes in test
    ]
    ...
    if num_victim_test_nodes > 0:
        num_test_poison = int(test_poison_fraction*num_victim_test_nodes)
